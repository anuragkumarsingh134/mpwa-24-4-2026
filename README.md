...
Copy `.env.example` to `.env` before running the install wizard. Most values are **auto-configured by the wizard**, but here's the full reference:

### App Settings

| Variable | Description | Default |
|----------|-------------|---------|
| `APP_NAME` | Application name | `MPWA` |
| `APP_ENV` | Environment (`production` or `local`) | `production` |
| `APP_KEY` | Laravel encryption key | Auto-generated by wizard |
| `APP_DEBUG` | Enable debug mode (`true`/`false`) | `false` |
| `APP_URL` | Your application URL | Set by wizard |
| `APP_INSTALLED` | Must be `false` for install wizard to appear | `false` |

### Server Configuration (set by install wizard)

| Variable | Description | Default |
|----------|-------------|---------|
| `TYPE_SERVER` | Server type: `localhost`, `hosting`, or `other` | `localhost` |
| `WA_URL_SERVER` | Full URL to Node.js WhatsApp server | `http://localhost:3100` |
| `PORT_NODE` | Node.js server port | `3100` |

### Database (set by install wizard)
...
| `DB_DATABASE` | Database name | `mpwa` |
| `DB_USERNAME` | Database username | `mpwa_user` |
...
| Variable | Description | Default |
|----------|-------------|---------|
| `CHATGPT_URL` | OpenAI API endpoint | `https://api.openai.com/v1/chat/completions` |
| `CHATGPT_MODEL` | ChatGPT model | `chatgpt-4o-latest` |
| `GEMINI_MODEL` | Google Gemini model | `gemini-2.0-flash` |
| `CLAUDE_URL` | Anthropic API endpoint | `https://api.anthropic.com/v1/messages` |
| `CLAUDE_MODEL` | Claude model | `claude-3-sonnet-20240229` |
| `DALLE_URL` | DALL·E image endpoint | `https://api.openai.com/v1/images/generations` |
| `DALLE_SIZE` | Generated image size | `256x256` |

> **Note:** AI API keys are configured per-device through the admin panel, not in `.env`.
...
| `MAIL_FROM_NAME` | Sender display name | `MPWA` |

### Other Settings

| Variable | Description | Default |
|----------|-------------|---------|
| `THEME_NAME` | Active admin theme | `vuexy` |
| `ENABLE_INDEX` | Show landing page (`yes`/`no`) | `yes` |
| `ORIGIN` | Allowed CORS origin | `http://localhost` |
| `WEBHOOK` | Global webhook URL for incoming messages | — |
| `TRIAL_MESSAGE_LIMIT` | Max messages for trial plans | `50` |
| `TRIAL_DEVICES_LIMIT` | Max devices for trial plans | `1` |
